...
import PromptInjectionChecker from "./PromptInjectionChecker";
...
const relatedLinks = [
  ...
  { href: "/tools/claude-desktop-mcp-config-generator/", label: "Claude Desktop MCP Config Generator" },
  ...
  { href: "/guides/best-mcp-servers-for-claude/", label: "Best MCP Servers for Claude" },
];
  ...
  const schema = [
    {
      ...
      featureList: [
        "Scan system prompts, agent prompts, and user inputs for injection risks",
        "Detect instruction override, secret leakage, jailbreak, and restriction bypass patterns",
        "Calculate heuristic risk scores for injection and secret leakage",
        "Provide suggested fixes and defense-in-depth recommendations",
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 3, name: "Prompt Injection Checker", item: tool.canonical },
      ],
    },
  ];
      ...
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
        <nav aria-label="Breadcrumb" className="text-sm text-slate-500">
          <ol className="flex items-center gap-2">
            ...
            <li className="text-slate-300">/</li>
            <li className="font-medium text-slate-900">Prompt Injection Checker</li>
          </ol>
        </nav>
      </div>
      ...
      <section className="bg-slate-950 py-16 sm:py-24">
        <div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-blue-600/20 px-4 py-1.5 text-sm font-medium text-blue-300">
            <Shield className="h-4 w-4" /> AI Security Scanner
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">Prompt Injection Checker</h1>
          <p className="mx-auto mt-6 max-w-2xl text-xl text-slate-300">Free AI security scanner for LLMs.</p>
      ...
      <section className="py-12">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8">
            <h2 className="text-2xl font-bold text-slate-950">What this scanner detects</h2>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {[
                { title: "Instruction Override", desc: "Patterns like 'ignore previous instructions' that attempt to replace system directives." },
                { title: "System Prompt Leakage", desc: "Requests to reveal or show the system prompt, hidden instructions, or internal rules." },
                { title: "Secret Extraction", desc: "Attempts to output secrets, API keys, passwords, or credentials embedded in prompts." },
                { title: "Jailbreak Patterns", desc: "Known jailbreak keywords such as DAN, 'do anything now', and related variants." },
                { title: "Restriction Bypass", desc: "Phrases that attempt to disable safety controls, remove restrictions, or bypass limits." },
                { title: "Role-Play Attacks", desc: "Combinations of role-play with instruction override to manipulate model behavior." },
              ].map((card) => (
                <div key={card.title} className="rounded-2xl border border-slate-200 p-5">
