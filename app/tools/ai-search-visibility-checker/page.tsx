...
import AISearchVisibilityChecker from "./AISearchVisibilityChecker";
...
const relatedLinks = [
  ...
  { href: "/pricing/", label: "Pro Reports" },
];
  ...
  const schema = [
    {
      ...
      featureList: ["AI search visibility scoring", "Answer engine optimization checklist", "Citation readiness report", "Local browser-only content audit"],
    },
    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <nav className="flex gap-6">
              ...
              <Link href="/workflows/" className="text-sm text-slate-600 hover:text-slate-900">Workflows</Link>
      ...
      <section className="bg-slate-950 py-16 sm:py-24">
        <div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-blue-600/20 px-4 py-1.5 text-sm font-medium text-blue-300"><SearchCheck className="h-4 w-4" /> AEO / GEO Checker</div>
      ...
      <AISearchVisibilityChecker />
      <section className="py-12">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-6 md:grid-cols-3">
            {["Score pages for entity clarity, use cases, proof, FAQs, and citation snippets.", "Get concrete fixes for ChatGPT, Perplexity, Gemini, and answer-engine readiness.", "Use a lightweight local checker before investing in full content production."].map((item) => <div key={item} className="rounded-2xl border border-slate-200 bg-white p-6"><CheckCircle2 className="h-5 w-5 text-blue-600" /><p className="mt-3 text-sm leading-6 text-slate-700">{item}</p></div>)}
          </div>
        </div>
      </section>
