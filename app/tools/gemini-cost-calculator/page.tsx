...
import ProviderCostCalculator from "../_components/ProviderCostCalculator";
...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="bg-slate-950 py-16 sm:py-24"><div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8"><div className="mb-6 inline-flex items-center gap-2 rounded-full bg-blue-600/20 px-4 py-1.5 text-sm font-medium text-blue-300"><Calculator className="h-4 w-4" /> Google AI Budgeting</div><h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">{tool.h1}</h1><p className="mx-auto mt-6 max-w-2xl text-xl text-slate-300">{tool.intro}</p><p className="mx-auto mt-4 max-w-3xl text-base leading-7 text-slate-400">{tool.description} Everything runs as a static browser tool with no login, database, or AI API call.</p><div className="mt-8 flex flex-wrap justify-center gap-3">{tool.tags.map((tag) => <span key={tag} className="rounded-full bg-white/10 px-3 py-1 text-sm text-slate-200">{tag}</span>)}</div><div className="mt-8"><Link href="#calculator" className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-700">Use the tool <ArrowRight className="h-4 w-4" /></Link></div></div></section>

      <ProviderCostCalculator provider="Google" defaultModelId="gemini-2-5-flash" sample={{ inputTokens: 8000, outputTokens: 1000, dailyRequests: 800, label: "Long-context sample" }} copy={{ eyebrow: "Gemini planning", title: "Estimate Gemini API spend", description: "Model Google Gemini costs for long-context assistants, multimodal workflows, and AI apps before production usage grows." }} />
