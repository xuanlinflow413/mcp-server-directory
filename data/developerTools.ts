...
export const developerTools: ToolConfig[] = [
...

  {
    ...
    shortName: "AI Costs",
    title: "AI Cost Calculator — Estimate OpenAI, Claude & LLM Costs",
    description: "Calculate token usage and estimate monthly AI costs across OpenAI, Claude, Gemini, DeepSeek and Kimi models.",
    canonical: `${base}/tools/ai-cost-calculator/`,
    h1: "AI Cost Calculator",
    intro: "Estimate AI API costs before you ship.",
    mode: "ai-cost",
    tags: ["ai cost calculator", "llm cost calculator", "token cost calculator", "openai costs"],
    sample: "",
    faqs: [
      { question: "What is token pricing?", answer: "Token pricing charges separately for input tokens sent to a model and output tokens generated by the model, usually measured per one million tokens." },
      { question: "How do OpenAI costs work?", answer: "OpenAI API costs are typically calculated from input tokens plus output tokens multiplied by the selected model's published per-token rates." },
      { question: "How much does Claude cost?", answer: "Claude pricing varies by model. Sonnet is usually priced for balanced production use, while Opus-class models cost more for higher reasoning capability." },
      { question: "How can I reduce AI costs?", answer: "Reduce AI costs by trimming prompts, caching repeated responses, routing simple tasks to cheaper models, limiting max output tokens, and monitoring daily request volume." },
      { question: "What is the cheapest LLM?", answer: "The cheapest LLM depends on your workload, context size, and output length. Mini and efficient models such as GPT-5 Mini or DeepSeek-style models are often cheaper for high-volume tasks." },
      { question: "How do I estimate monthly usage?", answer: "Estimate monthly usage by multiplying average input tokens, output tokens, and daily requests by 30 days, then applying the model's input and output token prices." },
      { question: "What is a token?", answer: "A token is a chunk of text used by language models. In English, one token is often around four characters, but the exact split depends on the tokenizer." },
